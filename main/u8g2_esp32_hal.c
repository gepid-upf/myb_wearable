...
static const char *TAGI = "u8g2_hal";
...
uint8_t u8g2_esp32_msg_comms_cb(u8x8_t *u8x8, uint8_t msg, uint8_t arg_int, void *arg_ptr) {
	//ESP_LOGD(TAGI, "msg_comms_cb: Received a msg: %d: %s", msg, msgToString(msg, arg_int, arg_ptr));
	switch(msg) {
		...
		case U8X8_MSG_BYTE_INIT: {
		  ...
		  //ESP_LOGI(TAGI, "... Initializing bus.");
		...
		case U8X8_MSG_BYTE_SEND: {
			...
			//ESP_LOGI(TAGI, "... Transmitting %d bytes.", arg_int);
...
uint8_t u8g2_esp32_msg_i2c_cb(u8x8_t *u8x8, uint8_t msg, uint8_t arg_int, void *arg_ptr) {
	...
	ESP_LOGD(TAGI, "msg_i2c_cb: Received a msg: %d %d", msg, arg_int);
	//ESP_LOGD(TAGI, "msg_i2c_cb: Received a msg: %d: %s", msg, msgToString(msg, arg_int, arg_ptr));
	...
	switch(msg) {
		...
		case U8X8_MSG_BYTE_INIT: {
			...
			ESP_LOGI(TAGI, "sda_io_num %d", u8g2_esp32_hal.sda);
		    conf.sda_io_num = u8g2_esp32_hal.sda;
		    conf.sda_pullup_en = GPIO_PULLUP_ENABLE;
			ESP_LOGI(TAGI, "scl_io_num %d", u8g2_esp32_hal.scl);
		    conf.scl_io_num = u8g2_esp32_hal.scl;
		    conf.scl_pullup_en = GPIO_PULLUP_ENABLE;
			ESP_LOGI(TAGI, "clk_speed %d", I2C_MASTER_FREQ_HZ);
		    conf.master.clk_speed = I2C_MASTER_FREQ_HZ;
			ESP_LOGI(TAGI, "i2c_param_config %d", conf.mode);
		    ESP_ERROR_CHECK(i2c_param_config(I2C_MASTER_NUM, &conf));
			ESP_LOGI(TAGI, "i2c_driver_install %d", I2C_MASTER_NUM);
...
/*
			...
			ESP_LOGI(TAGI, "i2c_master_cmd_begin %d", I2C_MASTER_NUM);
...
*/
		...
		case U8X8_MSG_BYTE_SEND: {
			...
			ESP_ERROR_CHECK(i2c_master_start(cmd));
//			ESP_LOGI(TAGI, "I2CAddress %02X", u8x8_GetI2CAddress(u8x8)>>1);
...
uint8_t u8g2_esp32_msg_gpio_and_delay_cb(u8x8_t *u8x8, uint8_t msg, uint8_t arg_int, void *arg_ptr) {
	//ESP_LOGD(TAGI, "msg_gpio_and_delay_cb: Received a msg: %d: %s", msg, msgToString(msg, arg_int, arg_ptr));
	switch(msg) {
...
uint8_t u8g2_esp32_msg_i2c_and_delay_cb(u8x8_t *u8x8, uint8_t msg, uint8_t arg_int, void *arg_ptr) {
	...
	ESP_LOGD(TAGI, "msg_i2c_and_delay_cb: Received a msg: %d", msg);
