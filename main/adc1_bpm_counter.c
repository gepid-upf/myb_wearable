...
/*
///////////////////////////// NOTAS ///////////////////////////////
array[10] - armazena +1 bpm a cada 10 segundos (se for válido). cada 2 minutos realiza média e envia ao array maior
Após enviar, esvazia o buffer. Se buffer cheio, sobrescreve a partir do 0. Se sobrar espaço, calcula quantos tem.

array[540] - armazena médias de bpm para um dia: 18 horas, cada 2 minutos um novo valor, quando encher o buffer, para de encher.

obs.: ao passar ao principal, pegar horário em q começou a armazenar, para enviar junto com os bpms
 e número de passos
*/
...
void task_sen0203(char *beatstep)
{
    ...
    while (1)
    {
        ...
        if (module(diff_variation) > 28)//desabilita beet se variação entre últimos 3 valores for maior q 25
            beet = false;

        if (variation > 28)//desabilitia beet se variação entre últimos 2 valores for maior que 25
            beet = false;
        ...
        if (module(diff_variation - last_diff_variation) > 33)//desabilita beet se variação entre últimos valores for maior que 30
        ...
        if (!last_beet && beet)//se a última leitura não for um batimento, e a atual for, detecta pulso
        {
            ...
            if (samples_bpm < N_SAMPLES_BPM) //usar um bool pra economizar memória
             {
                if(samples_bpm == 0)
             ...
             {
                //reseta congagem de pulsos, registra tempo do segundo pulso.
                current_time = (uint32_t)(xTaskGetTickCount() * portTICK_PERIOD_MS);
                //Faz a diferença de tempos, obtendo o período
                diff = current_time - up_time;

                bpm = ((samples_bpm * 60000.0) / diff);

                samples_bpm = 0;
                //tmax == 1200ms && tmin == 333ms
                //se contagem de bpm não for aceitável, zera
